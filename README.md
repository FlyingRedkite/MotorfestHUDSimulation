...
Prototype technique d'un HUD multijoueur sur Unreal Engine 5.7, entièrement en C++. Focus sur l'architecture réseau UE5 : 
- réplication de variables
- Server/Client RPC 
- Serveur autorité. 

Pas de physique voiture — le projet cible délibérément la couche réseau et UI, pas la mécanique de jeu.
...
Projet de suivi du simulateur ImGui (https://github.com/FlyingRedkite/ImGUI_HUD_Simulation), cette fois dans un contexte moteur réel.

L'objectif : transposer les concepts de synchronisation entre threads (mutex, copie locale, autorité d'écriture) vers les mécanismes de réplication UE5, et construire un HUD par client affichant un classement dépendant de la position des joueurs par 
rapport à la ligne d'arrivée ainsi que leur vitesse réelle (découplé du serveur).
...
Un serveur autoritaire UE5 fait tourner une partie avec 2 à 4 véhicules contrôlables. Chaque client dispose d'un HUD indépendant affichant sa vitesse locale, le classement général et le ping de chaque joueur.
...
* lue depuis le Character local, sans passer par le réseau
```
...
| `Motorfest_HUD_multiCharacter` | Véhicule contrôlable, envoie sa vitesse et distance au serveur | Serveur + tous clients |
...
### HUD et performance
- Vitesse locale lue depuis `GetVelocity()` sur le Character — zéro latence réseau, pas de round-trip serveur
- Classement reconstruit toutes les 500ms, pas à chaque frame
- Delta compression sur les RPC de vitesse (seuil 5 cm/s) afin de ne pas surcharger le réseau
...
| Packet loss simulé (5%) | Network Emulation UE5 | Se rendre compte de l'impact du réseau sur l'expérience |
...
3. Lancer le projet avec les options multiplayer : 
Number of players = 2-4 et Net mode : Play as listen server
